...
sidebarPanel(
#Sliders:
sliderInput("beta", "Transmission (yr^-1):", 5,
              min = 0, max = 10),
sliderInput("infper", "Infectious period (days)", 3000,
              min = 1, max = 5000),
sliderInput("mu", "birth rate:", 5,
              min = 0, max = 100),
sliderInput("T", "Time range:",
                  min = 0, max = 10, value = c(0,1)),
width=3
),
...
mainPanel(
  ...
  tabsetPanel(
      ...
      tabPanel("Equations", 
           withMathJax(
            helpText("Susceptible \n $$\\frac{dS}{dt} = \\underbrace{\\mu N}_{birth} - \\underbrace{\\mu S})_{death} - \\underbrace{\\frac{\\beta I S}{N}_{infection}$$ \n Infecitous \n $$\\frac{dI}{dt} = \\frac{\\beta I S}{N} - (\\mu+\\sigma) I$$"),
